...
module "glue_db" {
  ...
  description = "OpenCode pipeline catalog (dev)"
}
...
module "runs_table" {
  ...
  name     = "runs"
  location = "s3://${module.clean_bucket.name}/curated/runs/"
  columns = [
...
module "run_steps_table" {
  ...
  database = module.glue_db.name
  name     = "run_steps"
  location = "s3://${module.clean_bucket.name}/curated/run_steps/"
  columns = [
    { name = "run_id", type = "string" },
...
module "verification_table" {
  ...
  location = "s3://${module.clean_bucket.name}/curated/verification_results/"
  columns = [
    { name = "run_id", type = "string" },
...
module "errors_table" {
  ...
  database = module.glue_db.name
  name     = "errors"
  location = "s3://${module.clean_bucket.name}/curated/errors/"
  columns = [
    { name = "run_id", type = "string" },
...
module "graph_table" {
  ...
  location = "s3://${module.clean_bucket.name}/curated/graph_snapshots/"
  columns = [
    { name = "run_id", type = "string" },
...
module "athena" {
  ...
  name           = "project15-dev"
  result_output  = "s3://${module.clean_bucket.name}/athena-results/dev/"
}
