...
export namespace LSP {
  ...
  export async function saveFile(input: string, waitForDiagnostics?: boolean) {
    log.info("saving file", { file: input })
    const clients = await getClients(input)
    await Promise.all(
      clients.map(async (client) => {
        const wait = waitForDiagnostics ? client.waitForDiagnostics({ path: input }) : Promise.resolve()
        await client.notify.save({ path: input })
        return wait
      }),
    ).catch((err) => {
      log.error("failed to save file", { err, file: input })
    })
    return clients.length
  }
